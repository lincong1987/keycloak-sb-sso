<template>
	<ul ref="list" @click.stop="selectedTime" :class="`${prefix}-time-item`">
		<li
			v-for="(item, index) in times"
			:key="item.value"
			:data-key="item.value"
			:class="[
				`${prefix}-time-item-child`,
				{ disabled: item.disabled, active: item.value === value, focus: focus === index }
			]"
		>
			{{item.value}}
		</li>
