...
function getPages(projectConfig) {
	return projectConfig.pages || {
		src: {
...
function findModules(pkg, projectConfig) {
	if (modules == null) {
		modules = {};
...
/**
 * parseAlias
 * @desc vue.config.js 别名生成器
 * @param {Object} pkg package.json 对象
 * @param {Object} projectConfig 项目配置文件
 * */
const parseAlias = function (pkg, projectConfig) {
	let alias = {};
	// 根据入口页面，定义不同的别名
	Object.keys(getPages(projectConfig)).forEach(page => {
		if (page === "src") {
			alias['@'] = resolve('src');
		} else {
...
/**
 * parseProxy
 * @desc vue.config.js devServer.proxy 配置项生成器
 * @param {Object} projectConfig 项目配置文件
 * */
const parseProxy = function (projectConfig) {
	let proxy = {};
	(projectConfig.service || []).forEach((service) => {
		...
		let p = Object.assign({}, {
			...
			bypass: (req, res, proxyConfig) => {
				...
				console.log(`${new Date().getDate()} [${req.method}] ${res.statusCode} ${host}${req.url}`)
			},
		}, service)
...
/**
 * parsePages
 * @desc vue.config.js pages 配置项生成器
 * @param {Object} projectConfig 项目配置文件
 * */
const parsePages = function(projectConfig) {
	let pages = {};
	// 页面入口
	let defaults = getPages(projectConfig);
	Object.keys(defaults).forEach(page => {
		pages[page] = Object.assign({
			// page 的入口
			entry: page + '/main.js', // 模板来源主文件
			filename: page + '.html', // 当使用 title 选项时，
			title: projectConfig.title, // 在这个页面中包含的块，默认情况下会包含
...
function parseCopy(pkg, projectConfig) {

	let r = []

	// 22.08.11---停止模块包覆盖文件操作 包里的文件会覆盖项目中的文件是个 bug

	// let m = findModules(pkg, projectConfig);
	...
	const pages = parsePages(projectConfig)
...
/**
 * getPlugin
 * @desc vue.config.js configureWebpack.plugins 配置项生成器
 * @param {Object} pkg package.json 对象
 * @param {Object} projectConfig 项目配置文件
 * */
const getPlugin = function(pkg, projectConfig) {
	let plugins = [];
	// 多模块下使用
	if ((pkg.modules || []).length > 0) {
		// 复制模块下的静态文件
		const copys = parseCopy(pkg, projectConfig);
 ...
 * @param {Object} pkg package.json 对象
 * @param {Object} projectConfig 项目配置文件
 * @param {Object} options 合并配置项
 * */
const DefineVueConfig = function (path, pkg, projectConfig, options = {}) {
	PROJECT_DIR = path
	try {
	...
	return Object.assign({
		...
		devServer: Object.assign({}, {
			...
			proxy: parseProxy(projectConfig),
			before: app => {
		...
		configureWebpack: {
			plugins: getPlugin(pkg, projectConfig),
			resolve: {
				alias: parseAlias(pkg, projectConfig)
			}
		},
		chainWebpack(config) {
			if (!FbAssets) return;
