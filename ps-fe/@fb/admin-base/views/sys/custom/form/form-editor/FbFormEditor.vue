...
<!--							</fb-button>-->
							...
							<fb-button type="primary" success icon="save" @on-click="doSave">保 存</fb-button>
						</template>
					</fb-card>
				</fb-container>
			</fb-layout-header>
						...
						>
							...
							<fb-property bordered mode="form">
								<fb-row>
									...
									<fb-col span="12">
										<fb-property-item label="uri">
											<fb-input :value="`${search.actions.add.uri}`" readonly></fb-input>
										</fb-property-item>
									</fb-col>
								</fb-row>
								<fb-row>
									...
									<fb-col span="12">
										<fb-property-item label="uri">
											<fb-input :value="`${search.actions.modify.uri}`" readonly></fb-input>
										</fb-property-item>
									</fb-col>
								</fb-row>
								<fb-row>
									...
									<fb-col span="12">
										<fb-property-item label="uri">
											<fb-input :value="`${search.actions.delete.uri}`" readonly></fb-input>
										</fb-property-item>
									</fb-col>
								</fb-row>
								<fb-row>
									...
									<fb-col span="12">
										<fb-property-item label="uri">
											<fb-input :value="`${search.actions.query.uri}`" readonly></fb-input>
										</fb-property-item>
									</fb-col>
								</fb-row>
								<fb-row>
									...
									<fb-col span="12">
										<fb-property-item label="uri">
											<fb-input :value="`${search.actions.export.uri}`" readonly></fb-input>
										</fb-property-item>
									</fb-col>
								</fb-row>
							</fb-property>
							...
							<fb-dialog title="添加表单至菜单" ref="add-to-menu">
								...
								<fb-card body-overflow="auto" v-autoheight="50">
									<fb-tree
										...
										:service="$svc.sys.menu.tree" :param="{menuId: ''}">

										<template #node="{node}">
											{{ node.label }}
											<fb-icon name="delete" color="red" @on-click.stop="()=>{
											  handleRemoveMenu(node)
											}"/>
										</template>
...
import FbFormEditorAddRow from '../../../../../views/sys/custom/form/form-editor/FbFormEditorAddRowContainer'
import FbFormEditorRowToolbar from '../../../../../views/sys/custom/form/form-editor/FbFormEditorRowToolbar'
import FbFormEditorCol from '../../../../../views/sys/custom/form/form-editor/FbFormEditorCol'
import FbFormEditorComponentPropsPanel from '../../../../../views/sys/custom/form/form-editor/FbFormEditorComponentPropsPanel'
import FbFormEditorRowViewToolbar from '../../../../../views/sys/custom/form/form-editor/FbFormEditorRowViewToolbar'
import FbFormEditorSearchPropsPanel from '../../../../../views/sys/custom/form/form-editor/FbFormEditorSearchPropsPanel'
import FbFormEditorSearchPanel from '../../../../../views/sys/custom/form/form-editor/FbFormEditorSearchPanel'
import FbFormEditorColAddPreview from '../../../../../views/sys/custom/form/form-editor/FbFormEditorColAddPreview'
...
import services from '../../../../../views/sys/custom/form/form-editor/utils/services'
import { each } from 'lodash-es/collection'
import getComponentsNameList from '../../../../../views/sys/custom/form/form-editor/utils/schema/getComponentsNameList'
...
export default {
	...
	methods: {
		async handleRemoveMenu (node) {
			let msg = ''
			if (node.children && node.children.length) {
				msg = `[${node.label}] 下还有子节点，确定删除吗？该操作会先删除所有的子节点且不可恢复！`
			} else {
				msg = `节点 [${node.label}] 将被删除且不可恢复，确定删除吗？`
			}

			this.$confirm(msg, async () => {
				await this.doRemoveMenu(node)
				this.$refs['add-to-menu-tree'].removeNode(node)
			})

		},
		async doRemoveMenu (node) {

			if (node.children && node.children.length) {

				each(node.children, async (child) => {
					await this.doRemoveMenu(child)
				})
				node.children = []
				setTimeout(async () => {
					await this.doRemoveMenu(node)
				...
				await this.$svc.sys.menu.delete({
					menuId: node.id,
					menuName: node.label,
					menuCode: node.extend,
					menuUri: node.menuUri,
					menuPid: node.pid,
					menuSourceName: node.menuSourceName,
					menuTypeName: node.menuTypeName,
					menuIcon: node.menuIcon,
					menuDesc: node.menuDesc,
					orderIndex: node.orderIndex,
					enabled: node.enabled,
				}).then(() => {
					this.$message.success(`${node.label} 删除成功！`)
				})
			}
		},
		handleAddToMenu (type) {
			...
			Object.values(this.search.actions).forEach(button => {
				buttons.push({
					...
					menuUri: encodeURIComponent(button.uri || ''),
					menuPid: '',
					menuSource: 1,
					menuType: 'SYS1903',
					menuIcon: '',
		...
		syncColByUuid (col) {
			...
			if (!['fb-fieldset', 'fb-text', 'fb-color-picker', 'tp-upload', ''].includes(col.type)) {
				if (!queryCol) {
					...
					let searchType = queryCol.formItemProps.searchType
			...
			let columnsCol = this.search.table.props.columns.find(n => !!n.item && col.uuid === n.item.uuid)
			if (!columnsCol) {
				let colModel = toRaw(col)
				let column = {
