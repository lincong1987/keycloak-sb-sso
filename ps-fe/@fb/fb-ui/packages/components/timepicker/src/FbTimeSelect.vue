<template>
    <div @keydown="keydown" :class="`${prefix}-time-select`" tabindex="0">
        ...
        <fb-time-item
            ...
            :value="selectedHour"
            :focus="focus['1']"
            @on-change="changeSelectedHour"
            :class="{
                [`${prefix}-time-item-focus`]: focusType === 1
            }"
        />
        <fb-time-item
            v-if="minutes"
            ...
            :focus="focus['2']"
            @on-change="changeSelectedMinute"
            :class="{
               [`${prefix}-time-item-focus`]: focusType ===  2
            }"
        />
        <fb-time-item
            v-if="seconds"
            ...
            :focus="focus['4']"
            @on-change="changeSelectedSeconds"
            :class="{
              [`${prefix}-time-item-focus`]: focusType === 3
            }"
        />
    </div>
</template>
...
import FbTimeItem from './FbTimeItem'
import { prefix } from '../../../../src/config'

export default {
    name: 'FbTimeSelect',
    components: {FbTimeItem},
    props: {
        value: {
            type: String,
            default: '',
    ...
    methods: {
        ...
        resetFromHistory (target) {
            return this.historyFocus[this.focusType] !== -1 ? this.historyFocus[this.focusType] : this.resetFindFocus(
                target, 0, 1)
        },
        changeCurrentFocus () {
            if (this.focusType & 1) {
