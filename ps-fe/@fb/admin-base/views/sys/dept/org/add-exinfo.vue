	...
	<div class="tp-dialog">
		<div class="tp-dialog-top">
			<fb-form ref="fbform" :label-width="120">
				<fb-row>
					<fb-col span="12">
						<fb-form-item label="直接监管上级部门">
							<fb-select v-model="formData.supdeptId"
									   :service="$svc.sys.dict.select"
									   :param="{'pdicCode': 'SYS1501'}"
									   placeholder="请选择"
									   clearable/>
						</fb-form-item>
					</fb-col>
				...
				<fb-row>
					<fb-col span="24">
						<fb-form-item label="主管行业代码">
							<fb-tree-select
								ref="direinDustryCodeTree"
								v-model="formData.direinDustryCode"
				...
				<fb-row>
					<fb-col span="24">
						<fb-form-item label="地址">
							<fb-input v-model="formData.address" type="text" placeholder="请输入内容">
							</fb-input>
						</fb-form-item>
					</fb-col>
				</fb-row>
		...
		<div class="tp-dialog-bottom">
			<fb-button style="margin-right: 12px" type="primary" @on-click="add">保存扩展信息</fb-button>
		...
		methods: {
			...
			view(deptId) {
				...
				this.service.expView({"deptId": deptId}).then((result) => {
					...
					if (result.code == 1) {
						if (result.data !== "") {
							result.data.direinDustryCode = result.data.direinDustryCode.split(",")
			...
			direinDustryChange(val, node) {

				if (node && node.length >= 1) {
					this.formData.direinDustryName = node.map(item => item.label);
				}
			},
