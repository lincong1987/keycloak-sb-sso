	...
	<div>
		<fb-page-search>
			...
			<template slot="buttons">
				<fb-button ref="buttonAdd" @on-click="handleAdd" v-permission="'SYS_ORG_ROLE_ADD'" icon="add-circle">
					新增
				</fb-button>
			</template>
			...
			<template slot="table">
				<fb-simple-table
					...
					<template v-slot:actions="props">
						<fb-space>
							<fb-button @on-click="handleEdit(props.row)" v-permission="'SYS_ORG_ROLE_UPDATE'"
									   editor size="s">修改</fb-button>
							<fb-button @on-click="handleAuth(props.row)" v-permission="'SYS_ORG_ROLE_AUTHADD'"
									   editor size="s">授权</fb-button>
							<fb-button @on-click="handleDel(props.row)" v-permission="'SYS_ORG_ROLE_DELETE'"
					...
					<template v-slot:view="props">
						<fb-link-group>
							<fb-link :click="()=>handleView(props.row)" :label="props.row.roleName" type="primary"></fb-link>
						</fb-link-group>
					</template>
				</fb-simple-table>
			</template>
	...
	export default {
		...
		methods: {
			...
			handleAdd() {
				...
				// 打开新增界面弹出窗
				this.$refs.TpDialog.show(import('../../../../views/sys/role/org/add.vue'), param, "新增", options);
			},
			// 修改方法
			handleEdit(row) {
				...
				this.$refs.TpDialog.show(import('../../../../views/sys/role/org/add.vue'), param, "修改", options);
			},
			// 删除方法
			handleDel(row) {
				this.table.service.selectByRoleid({"roleId": row.roleId, "passKey": row.passKey}).then((result) => {
					// 判断code
					if (result.code == 1) {
						if (result.data != 0) {
							this.$confirm('该角色已经有人员绑定，请慎重操作！确定要删除吗？', () => {
								this.delete(row.roleId, row.creator);
							})
						} else {
							this.$confirm('确定要删除吗？', () => {
								this.delete(row.roleId, row.creator, row.passKey);
							})
						}
					} else {
						// 服务器返回失败
