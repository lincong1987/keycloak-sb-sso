...
(function () {
	...
	if (!window.methodDraw) window.methodDraw = function ($) {
		...
		curConfig = {
			...
			initOpacity: 1,
			imgPath: './Plugins/method-draw/images/',
			extPath: './Plugins/method-draw/extensions/',
			jGraduatePath: './Plugins/method-draw/lib/jgraduate/images/',
			extensions: [],
			initTool: 'select',
			wireframe: false,
			...
			uiStrings = Editor.uiStrings = {
				...
				// This is needed if the locale is English, since the locale
				// strings are not read in that instance.
				layers: {
					'layer': 'Layer',
				},
				notification: {
		...
		// Extension mechanisms must call setCustomHandlers with two functions:
		// opts.open and opts.save opts.open's responsibilities are: - invoke a
		// file chooser dialog in 'open' mode - let user pick a SVG file -
		// calls setCanvas.setSvgString() with the string contents of that file
		// opts.save's responsibilities are: - accept the string contents of
		// the current document - invoke a file chooser dialog in 'save' mode -
		...
		Editor.init = function () {
			...
			(function () {
				...
				if (w) {
					try {
						var methodDrawReadyEvent = w.document.createEvent(
							'Event')
						methodDrawReadyEvent.initEvent('methodDrawReady', true,
							true)
			...
			$.svgIcons(curConfig.imgPath + 'svg_edit_icons.svg', {
				...
				callback: function (icons) {
					...
					if (tleft.length != 0) {
						var min_height = tleft.offset().top +
							tleft.outerHeight()
					}
					...
					$('.tools_flyout').each(function () {
						...
						if (!shower.children('svg, img').length) {
							...
							if (clone.length) {
								clone[0].removeAttribute('style') //Needed for
								// Opera
								shower.append(clone)
							}
						}
			...
			Editor.canvas = svgCanvas =
				new $.SvgCanvas(document.getElementById('svgcanvas'),
					curConfig)
			Editor.show_save_warning = false
			Editor.paintBox = {
				fill: null,
			...
			// This sets up alternative dialog boxes. They mostly work the same
			// way as their UI counterparts, expect instead of returning the
			// result, a callback needs to be included that returns the result
			// as its first parameter. In the future we may want to add
			// additional types of dialog boxes, since they should be easy to
			// handle this way.
			(function () {
				$('#dialog_container').
					draggable({
				...
				var dbox = function (type, msg, callback, defText) {
					...
					var ok = $(
						'<input type="button" value="' + uiStrings.common.ok +
						'">').appendTo(btn_holder)
			...
			var setSelectMode = function () {
				...
				if (curr.length && curr[0].id !== 'tool_select') {
					curr.removeClass('tool_button_current').
			...
			// used to make the flyouts stay on the screen longer the very
			// first time
			...
			var saveHandler = function (window, svg) {
				...
				// by default, we add the XML prolog back, systems integrating
				// SVG-edit (wikis, CMSs) can just provide their own custom
				// save handler and might not want the XML prolog
				svg = '<?xml version="1.0"?>\n' + svg

				// Opens the SVG in new window, with warning about Mozilla bug
				// #308590 when applicable

				var ua = navigator.userAgent

				// Chrome 5 (and 6?) don't allow saving, show source instead (
				...
				var win = window.open(
					'data:image/svg+xml;base64,' + Utils.encode64(svg))

				// Alert will only appear the first time saved OR the first
				// time the bug is encountered
				var done = $.pref('save_notice_done')
				if (done !== 'all') {

					var note = uiStrings.notification.saveFromBrowser.replace(
						'%s', 'SVG')
			...
			var exportHandler = function (window, data) {
				...
				canvg(c, data.svg, {
					renderCallback: function () {
						...
						function downloadImg (datauri) {
							var img = document.createElement('img') // 获取要下载的图片
							img.src = datauri                            // 获取图片地址
							var a = document.createElement('a')          // 创建一个a节点插入的document
							var event = new MouseEvent('click')           // 模拟鼠标click点击事件
							a.download = 'beautifulGirl'                  // 设置a节点的download属性值
							a.href = datauri                                 // 将图片的src赋值给a节点的href
						...
						// 'PNG');  // Check if there's issues
						// if(issues.length) { var pre = "\n \u2022 "; note +=
						// ("\n\n" + uiStrings.notification.noteTheseIssues +
						// pre + issues.join(pre)); }  // Note that this will
						// also prevent the notice even though new issues may
						// appear later. // May want to find a way to deal with
						// that without annoying the user
			...
			var selectedChanged = function (window, elems) {
				...
				if (svgCanvas.elementsAreSame(multiselected)) selectedElement =
					multiselected[0]
			...
			var elementTransition = function (window, elems) {
				...
				multiselected =
					(elems.length >= 2 && elems[1] != null) ? elems : null
...
//              break;
			...
			var elementChanged = function (window, elems) {
				...
				for (var i = 0; i < elems.length; ++i) {
					...
					// if the element changed was the svg, then it could be a
					// resolution change
					if (elem && elem.tagName === 'svg') {
						//populateLayers();
						updateCanvas()
					}
						// Update selectedElement if element is no longer part
						// of the image.
					// This occurs for the text elements in Firefox
					else if (elem && selectedElement &&
						selectedElement.parentNode == null) {
//            || elem && elem.tagName == "path" && !multiselected) { // This
// was added in r1430, but not sure why
			...
			var zoomChanged = function (window, bbox, autoCenter) {
				...
				var z_info = svgCanvas.setBBoxZoom(bbox,
					w_area.width() - scrbar, w_area.height() - scrbar)
				if (!z_info) return
				...
				if (typeof animatedZoom !=
					'undefined') window.cancelAnimationFrame(animatedZoom)
				// zoom duration 500ms
				var start = Date.now()
				var duration = 500
				...
				var animateZoom = function (timestamp) {
					...
					if (tick < 1 && tick > -.90) {
						...
						$('option[value=' + parseInt(zoomlevel * 100) + ']',
							'#zoom_select').attr('selected', 'selected')
					}
				}
				animateZoom()
				...
				//  updateCanvas();
				//} else {
				//  updateCanvas(false, {x: bb.x * zoomlevel + (bb.width *
				// zoomlevel)/2, y: bb.y * zoomlevel + (bb.height *
				// zoomlevel)/2}); }
			...
			var setupFlyouts = function (holders) {
				$.each(holders, function (hold_sel, btn_opts) {
					...
					buttons.addClass('tool_button').
						unbind(
							'click mousedown mouseup') // may not be necessary
						.each(function (i) {
							...
							// Clicking the icon in flyout should set this
							// set's icon
							var func = function (event) {
								var options = opts
								//find the currently selected tool if comes
								// from keystroke
								if (event.type === 'keydown') {
									var flyoutIsSelected = $(
										options.parent + '_show').
										hasClass('tool_button_current')
									var currentOperation = $(
										options.parent + '_show').
										attr('data-curopt')
									$.each(holders[opts.parent],
										function (i, tool) {
								...
								shower.children(':not(.flyout_arrow_horiz)').
									remove()
								shower.append(icon).
									attr('data-curopt', options.sel) // This
								// sets
								// the
								// current
								// mode
					...
					shower.mousedown(function (evt) {
						...
						if ($('#tools_shapelib').
							is(':visible')) toolButtonClick(show_sel, false)
			...
			var extAdded = function (window, ext) {
				...
				var cb_ready = true // Set to false to delay callback (e.g.
				// wait for $.svgIcons)
				...
				if (ext.context_tools) {
					$.each(ext.context_tools, function (i, tool) {
						// Add select tool
						var cont_id = tool.container_id ? (' id="' +
							tool.container_id + '"') : ''
						...
						// TODO: Allow support for other types, or adding to
						// existing tool
						switch (tool.type) {
							case 'tool_button':
								var html = '<div class="tool_button">' +
									tool.id + '</div>'
							...
							case 'select':
								...
								html += '</select></label>'
								// Creates the tool, hides & adds it, returns
								// the select element
								var sel = $(html).
									appendTo(panel).
									find('select')
							...
							case 'button-select':
								...
								var list = $(
									'<ul id="' + tool.id + '_opts"></ul>').
									appendTo('#option_lists')
								if (tool.colnum) {
				...
				if (ext.buttons) {
					...
					$.each(ext.buttons, function (i, btn) {
						...
						if (!btn.includeWith && !btn.list) {
							if ('position' in btn) {
								...
								if (btn.type != 'menu' ||
									!btn.after) button.appendTo(parent)
							...
							if (btn.type == 'mode_flyout') {
								...
								// Create a flyout menu if there isn't one
								// already
								if (!ref_btn.parent().
									hasClass('tools_flyout')) {
									// Create flyout placeholder
									var tls_id = ref_btn[0].id.replace('tool_',
										'tools_')
									...
									// Create a flyout div
									flyout_holder =
										makeFlyoutHolder(tls_id, ref_btn)
									flyout_holder.data('isLibrary', true)
									show_btn.data('isLibrary', true)
								}

								//              var ref_data =
								// Actions.getButtonData(opts.button);

								placement_obj['#' + tls_id + '_show'] = btn.id
								// TODO: Find way to set the current icon using
								// the iconloader if this is not default

								// Include data for extension button as well as
								// ref button
							...
							} else if (btn.type == 'app_menu' || btn.type ==
								'menu') {
							...
							if (btn.isDefault) {
								$('#cur_' + btn.list).
									append(button.children().clone())
								var svgicon = btn.svgicon
									? btn.svgicon
									: btn.id
							...
							// TODO: Find way to set the current icon using the
							// iconloader if this is not default

							// Include data for extension button as well as ref
							// button
							var cur_h = holders['#' + flyout_holder[0].id] = [
								{
									sel: '#' + id,
									...
									icon: btn.id,
									key: btn.key,
									isDefault: btn.includeWith
										? btn.includeWith.isDefault
										: 0,
								}, ref_data,
							]

							// {sel:'#tool_rect', fn: clickRect, evt:
							// 'mouseup', key: 4, parent: '#tools_rect', icon:
							// 'rect'}

							var pos = ('position' in opts)
								? opts.position
								: 'last'
							...
							$.each(btn.events, function (name, func) {
								if (name == 'click') {
									if (btn.type == 'mode') {
										...
										if (btn.key) {
											$(document).
												bind('keydown', btn.key, func)
											if (btn.title) button.attr('title',
												btn.title + ' [' + btn.key +
												']')
										}
									} else {
					...
					$.svgIcons(svgicons, {
						...
						callback: function (icons) {
							// Non-ideal hack to make the icon match the
							// current size
			...
			if (!document.getElementById(
				'canvas_background')) createBackground()
			var fill = document.getElementById('canvas_background').
				getAttribute('fill')

			// updates the toolbar (colors, opacity, etc) based on the selected
			// element This function also updates the opacity and id elements
			// that are in the context panel
			var updateToolbar = function () {
					...
					switch (selectedElement.tagName) {
						...
						case 'a':
							...
							for (var i = 0, len = childs.length; i < len; i++) {
								var swidth = childs[i].getAttribute(
									'stroke-width')
								if (i === 0) {
									gWidth = swidth
						...
						default:
							...
							$('#stroke_width').
								val(selectedElement.getAttribute(
									'stroke-width') || 0)
							var dash = selectedElement.getAttribute(
								'stroke-dasharray') || 'none'
							$('option', '#stroke_style').removeAttr('selected')
							$('#stroke_style option[value="' + dash + '"]').
				...
				if (selectedElement != null) {
					var opac_perc = ((selectedElement.getAttribute('opacity') ||
						1.0) * 100)
			...
			var updateContextPanel = function (e) {
				...
				if (multiselected && multiselected[0] != null &&
					!multiselected[0].parentNode) multiselected = false

				var currentLayerName = svgCanvas.getCurrentDrawing().
					getCurrentLayerName()
				var currentMode = svgCanvas.getMode()
				var unit = curConfig.baseUnit !== 'px'
					? curConfig.baseUnit
					: null
				var is_node = currentMode == 'pathedit' //elem ? (elem.id &&
				// elem.id.indexOf('pathpointgrip')
				// == 0) : false;
				...
				if (is_node) {
					...
					$('#tool_add_subpath').
						...
						addClass('tool_button')
					$('#tool_node_delete').
						toggleClass('disabled', !path.canDeleteNodes)

					// Show open/close button based on selected point
					setIcon('#tool_openclose_path',
				...
				if (elem != null) {
					...
					if (!is_node && currentMode != 'pathedit') {
						...
						if (no_path) $('.action_path_convert_selected').
							removeClass('disabled')
						if (elname === 'path') $('.action_path_selected').
							removeClass('disabled')
					...
					if (panels[el_name]) {
						...
						$.each(cur_panel, function (i, item) {
							...
							var name_item = document.getElementById(
								el_name + '_' + item)
							name_item.value = Math.round(attrVal) || 0
							if (name_item.getAttribute('data-cursor') ===
								'true') {
						...
						if (el_name == 'text') {
							...
							$('#tool_italic').
								toggleClass('active', svgCanvas.getItalic())
							$('#tool_bold').
								toggleClass('active', svgCanvas.getBold())
					...
					menu_items[(el_name === 'g' ? 'en' : 'dis') +
					'ableContextMenuItems']('#ungroup')
					menu_items[((el_name === 'g' || !multiselected)
						? 'dis'
						: 'en') + 'ableContextMenuItems']('#group')
				}

				if (multiselected) {
					$('#multiselected_panel').show()
					$('.action_multi_selected').removeClass('disabled')
					menu_items.enableContextMenuItems('#group').
			...
			// bind the selected event to our function that handles updates to
			// the UI
...
//      var str = '<div class="palette_item transparent"
// data-rgb="none"></div>\
...
// ';" data-rgb="' + item + '"></div>'; }); $('#palette').append(str);

			let colors = ['#FF0000', '#0000FF', '#FFA500', '#FFFF00', '#000000']

			var colorsStr = '<div class="palette_item transparent"' +
				' style="position: static; width: 26px; height: 26px; margin-right:' +
				' 10px;' +
				' border: 1px' +
				' solid #D3D3D3;' +
			...
			colors.forEach(function (item, i) {
				colorsStr +=
					'<div class="palette_item" style=" margin-right: 10px;width: 26px; height:' +
					' 26px; border: 1px solid #D3D3D3; background-color: ' +
					item + ';" data-rgb="' + item + '"></div>'
			})

			$('#palette').append(colorsStr)
			...
			var changeStrokeWidth = function (ctl) {
				var val = ctl.value
				if (val == 0 && selectedElement &&
					['line', 'polyline'].indexOf(selectedElement.nodeName) >=
					0) {
					val = ctl.value = 1
				}
			...
			var operaRepaint = function () {
				// Repaints canvas in Opera. Needed for stroke-dasharray change
				// as well as fill change
			...
			// Lose focus for select elements when changed (Allows keyboard
			// shortcuts to work better)
			...
			$('#palette').
				on('mousemove mousedown touchstart touchmove', '.palette_item',
					function (evt) {
						evt.preventDefault()
						...
						if (picking) {
							...
							// Webkit-based browsers returned 'initial' here
							// for no stroke
			...
			// This is a common function used when a tool has been clicked
			// (chosen) It does several common things: - removes the
			// tool_button_current class from whatever tool currently has it -
			// hides any flyouts - adds the tool_button_current class to the
			// button passed in
			...
			var closer = function (e) {
				if (e.target.nodeName && e.target.nodeName.toLowerCase() ===
					'input') return false
				if (!$(e.target).hasClass('menu_title') &&
					!$(e.target).parent().hasClass('menu_title')) {
					if (!$(e.target).hasClass('disabled') &&
						$(e.target).hasClass('menu_item')) blinker(e)
			...
			$('.menu_title').on('mousedown', function () {
				$('#tools_shapelib').hide()
			...
			Editor.addDropDown = function (elem, callback, dropUp) {
				if ($(elem).length == 0) return // Quit if called on
			...
			var deleteSelected = function () {
				if (selectedElement != null || multiselected) {
					// console.log('delete dom---------------', selectedElement)
					var data = {
						id: selectedElement.id,
						attr: JSON.parse(JSON.stringify(selectedElement.dataset)),
						type: 'svg-remove'
					}
			...
			var pasteSelected = function () {
				flash($('#edit_menu'))
				var zoom = svgCanvas.getZoom()
				var x = (workarea[0].scrollLeft + workarea.width() / 2) / zoom -
					svgCanvas.contentW
				var y = (workarea[0].scrollTop + workarea.height() / 2) / zoom -
					svgCanvas.contentH
			...
			var convertToPath = function () {
				if (selectedElement != null) {
					...
					svgCanvas.selectorManager.requestSelector(elems[0]).
						reset(elems[0])
					svgCanvas.selectorManager.requestSelector(elems[0]).
						selectorRect.
			...
			var makeHyperlink = function () {
				if (selectedElement != null || multiselected) {
					$.prompt(uiStrings.notification.enterNewLinkURL, 'http://',
						function (url) {
			...
			var moveSelected = function (dx, dy) {
				if (selectedElement != null || multiselected) {
					if (curConfig.gridSnapping) {
						...
						var multi = svgCanvas.getZoom() *
							curConfig.snappingStep
						dx *= multi
						dy *= multi
					}
					$('input').blur()
			...
			var linkControlPoints = function () {
				//  var linked =
				// document.getElementById('tool_node_link').checked;
			...
			var addSubPath = function () {
				...
				if (sp) {
					button.addClass('push_button_pressed').
						removeClass('tool_button')
				} else {
					button.removeClass('push_button_pressed').
						addClass('tool_button')
				}
			...
			var clickExport = function () {
				var svgXml = svgCanvas.getSvgString();
				var resolution = svgCanvas.getResolution()
				var image = new Image();
				image.src = 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(svgXml))); //给图片对象写入base64编码的svg流
				...
				//   exportWindow =
				// window.open("data:text/html;charset=utf-8,<title>" + str +
				// "<\/title><h1>" + str + "<\/h1>"); }
			...
			var clickAlign = function () {
				...
				svgCanvas.alignSelectedElements(letter,
			...
			var clickSwitch = function () {
				...
				var fill_opacity = parseFloat(
					fill_rect.getAttribute('fill-opacity'))
			...
			var clickWireframe = function () {
				...
				if (!wf_rules.length) {
					wf_rules = $('<style id="wireframe_rules"><\/style>').
						appendTo('head')
				} else {
					wf_rules.empty()
				}
			...
			var updateWireFrame = function () {
				...
				var rule = '#workarea.wireframe #svgcontent * { stroke-width: ' +
					1 / svgCanvas.getZoom() + 'px; }'
				$('#wireframe_rules').
					text(workarea.hasClass('wireframe') ? rule : '')
			}
			...
			var saveSourceEditor = function () {
				...
				if (!svgCanvas.setSvgString($('#svg_source_textarea').val())) {
					$.confirm(uiStrings.notification.QerrorsRevertToSource,
						function (ok) {
							if (!ok) return false
							saveChanges()
						})
...
//        if(color == curPrefs.bkgd_color && url == curPrefs.bkgd_url) return;
			...
			var setIcon = Editor.setIcon =
				function (elem, icon_id, forcedSize) {
					var icon = (typeof icon_id === 'string') ? $.getSvgIcon(
						icon_id, true) : icon_id.clone()
					if (!icon) {
...
//        return;
//        var elems = $('.tool_button, .push_button, .tool_button_current,
// .disabled, .icon_label, #url_notice, #tool_open');
        ...
        #g_panel > *, #tool_font_size > *, .tools_flyout'
				...
				if (!rule_elem.length) {
					rule_elem = $('<style id="tool_size_rules"><\/style>').
						appendTo('head')
				} else {
					rule_elem.empty()
				}
				...
				if (size != 'm') {
					...
					var prefix = '-' + ua_prefix.toLowerCase() + '-'
					style_str +=
						(sel_toscale + '{' + prefix + 'transform: scale(' +
							scale + ');}'
							+ ' #svg_editor div.toolset .toolset {' + prefix +
							'transform: scale(1); margin: 1px !important;}' // Hack
							// for
							// markers
							+ ' #svg_editor .ui-slider {' + prefix +
							'transform: scale(' + (1 / scale) + ');}' // Hack
						                                              // for
						                                              // sliders
						)
			...
			var cancelOverlays = function () {
				...
				if (editingsource) {
					if (orig_source !== $('#svg_source_textarea').val()) {
						$.confirm(uiStrings.notification.QignoreSourceChanges,
							function (ok) {
			...
			(function () {
				workarea.scroll(function () {
					// TODO:  jQuery's scrollLeft/Top() wouldn't require a null
					// check
					if ($('#ruler_x').length != 0) {
			...
			function promptImgURL () {
				...
				$.prompt(uiStrings.notification.enterNewImgURL, curhref,
					function (url) {
						if (url) setImageURL(url)
					})
			}

			// TODO: go back to the color boxes having white background-color
			// and then setting background-image to none.png (otherwise
			// partially transparent gradients look weird)
			var colorPicker = function (elem) {
				var picker = elem[0].id == 'stroke_color' ? 'stroke' : 'fill'
				var is_background = elem[0].id == 'canvas_color'
				if (is_background) picker = 'canvas'
//        var opacity = (picker == 'stroke' ? $('#stroke_opacity') :
// $('#fill_opacity'));
				var paint = Editor.paintBox[picker].paint

				var title = (picker == 'stroke'
			...
			var PaintBox = function (container, type) {
				...
				if (type == 'canvas' && background) {
					var rgb = background.getAttribute('fill').
						match(/^rgb\((\d+),\s*(\d+),\s*(\d+)\)$/)
					if (rgb) {
						var hex = ('0' +
								parseInt(rgb[1], 10).toString(16)).slice(-2) +
          ...
          <defs><linearGradient id="gradbox_"/></defs></svg>', 'text/xml')
				...
				this.setPaint = function (paint, apply, noUndo) {
					...
					switch (ptype) {
						case 'solidColor':
							fillAttr =
								(paint[ptype] == 'none' || paint[ptype] ==
									'one') ? 'none' : '#' + paint[ptype]
							break
						case 'linearGradient':
						case 'radialGradient':
					...
					if (this.type == 'canvas') {
						...
						if (background) {
							...
							if (fillAttr.indexOf('url') ==
								-1) background.setAttribute('fill', fillAttr)
				...
				this.update = function (apply) {
					...
					switch (selectedElement.tagName) {
						...
						case 'image':
						case 'foreignObject':
							// These elements don't have fill or stroke, so
						...
						default:
							...
							var paintColor = selectedElement.getAttribute(
								type) || defColor
					}
					if (apply) {
			...
			var supportsNonSS = (test_el.style.vectorEffect ===
				'non-scaling-stroke')
			test_el.removeAttribute('style')
			var svgdocbox = Editor.paintBox.fill.rect.ownerDocument
			// Use this to test support for blur element. Seems to work to test
			// support in Webkit
			...
			// Test for embedImage support (use timeout to not interfere with
			// page load)
			setTimeout(function () {
				svgCanvas.embedImage(
					'../Plugins/method-draw/images/placeholder.svg',
					function (datauri) {
						if (!datauri) {
							// Disable option
							$('#image_save_opts [value=embed]').
			...
			var centerCanvas = function () {
				// this centers the canvas vertically in the workarea
				// (horizontal handled in CSS)
			...
			$('#resolution').change(function () {
				...
				if (!this.selectedIndex) {
					...
					var animateCanvasSize = function (timestamp) {
						...
						tick = (Math.pow((tick - 1), 3) + 1)
						w.value =
							(dims[0] - diff_w + (tick * diff_w)).toFixed(0)
						h.value =
							(dims[1] - diff_h + (tick * diff_h)).toFixed(0)
						changeCanvasSize()
						if (tick >= 1) {
							var res = svgCanvas.getResolution()
							...
							$('#resolution_label').
								html('<div class=\'pull\'>' + res.w +
									'<span>×</span></br>' + res.h + '</div>')
						} else {
			...
			// Associate all button actions as well as non-button keyboard
			// shortcuts
			var Actions = function () {
				// sel:'selector', fn:function, evt:'event', key:[key,
				// preventDefault, NoDisableInInput]
				var tool_buttons = [
					{
						sel: '#tool_select',
						fn: clickSelect,
					...
					// icon: 'circle'}, {sel:'#tool_fhellipse', fn:
					// clickFHEllipse, evt: 'mouseup', parent:
					// '#tools_ellipse', icon: 'fh_ellipse'},
					{
						sel: '#tool_path',
						fn: clickPath,
						evt: 'click',
					...
					//{sel:'#sidepanel_handle', fn: toggleSidePanel, key:
					// ['X']},
					{
						sel: '#copy_save_done',
						fn: cancelOverlays,
				...
				return {
					setAll: function () {
						...
						$.each(tool_buttons, function (i, opts) {
							...
							if (opts.sel) {
								var btn = $(opts.sel)
								if (btn.length == 0) return true // Skip if
								// markup
								// does not
								// exist
								if (opts.evt) {
									if (svgedit.browser.isTouch() &&
										opts.evt === 'click') opts.evt =
										'mousedown'
									btn[opts.evt](opts.fn)
								}

								// Add to parent flyout menu, if able to be
								// displayed
								if (opts.parent &&
									$(opts.parent + '_show').length != 0) {
									var f_h = $(opts.parent)
									if (!f_h.length) {
										f_h = makeFlyoutHolder(
											opts.parent.substr(1))
									}
							...
							if (opts.key) {
								...
								var keyval, shortcut = '', disInInp = true,
									fn = opts.fn, pd = false
								if ($.isArray(opts.key)) {
									keyval = opts.key[0]
									if (opts.key.length > 1) pd = opts.key[1]
									if (opts.key.length > 2) disInInp =
										opts.key[2]
								} else {
									keyval = opts.key
								}
								...
								if (svgedit.browser.isMac &&
									...
									if (modifier_key == 'ctrl') keyval.replace(
										'ctrl', 'cmd')
								}
...
//
			$('#image_hide').click(function () {
				svgCanvas

				var hide = svgCanvas.attr('hide') || '0'
				var _hide = hide === '0' ? '1' : '0'

				svgCanvas.attr('hide', _hide)
				//	updateContextPanel()

				$('#image_hide').
					toggleClass('active', hide === '0')
			})
//
					...
					switch (action) {
						...
						default:
							if (svgedit.contextmenu &&
								...
								svgedit.contextmenu.getCustomHandler(action).
									call()
							}
							break
					}
			...
			// 设置背景图
			Editor.setImageBackground = function (url) {
				...
				// lets insert the new image until we know
				// its dimensions
				var insertNewImage = function (imgBase, img_width, img_height) {
						...
						// console.log(newImage, 111111111111111111111)
						newImage.setAttribute(`data-bg`, 1)
						svgCanvas.setHref(newImage, imgBase)
						svgCanvas.selectOnly([newImage])
						// 移动到最后操作
						moveToBottomSelected()
						var canvas_background = $('#canvas_background')[0]
						svgCanvas.selectOnly([canvas_background])
						moveToBottomSelected()
				...
				// put a placeholder img so we know the
				// default dimensions
				var img_width = 100
				var img_height = 100
				var img = new Image()
				...
				img.onload = function () {
					var imgBase = getBase64Image(img)
					var width = img.width;
					var height = img.height;
					img_width = width //img.offsetWidth
					img_height = height // img.offsetHeight
					insertNewImage(imgBase, img_width, img_height)
			...
			// 设置小图标
			Editor.setImageIcon = function (url, imgWidth, imgHeight) {
				function getBase64Image(img) {
					var canvas = document.createElement("canvas");
					canvas.width = img.width;
					canvas.height = img.height;
				...
				// its dimensions
				var insertNewImage = function (imgBase, img_width, img_height) {
					// console.log(selectedElement, svgCanvas.getBBox(selectedElement), 11111111111)

					var bbox = {}
					if (selectedElement) {
						bbox = svgCanvas.getBBox(selectedElement)
					}
					var newImage = svgCanvas.addSvgElementFromJson(
						{
							'element': 'image',
				...
				img.onload = function () {
					...
					var width = img.width;
					var height = img.height;
					img_width = imgWidth || width //img.offsetWidth
					img_height = imgHeight || height // img.offsetHeight
					insertNewImage(imgBase, img_width, img_height)
					document.body.removeChild(img)
				}
			}
			...
			// 设置文本
			Editor.setAccompanyText = function (text, option) {
				if (selectedElement) {
					var bbox = svgCanvas.getBBox(selectedElement)
					...
					var newText = svgCanvas.addSvgElementFromJson(settings)
				}
			}
			...
			if (window.FileReader) {
						...
						if (file.type.indexOf('image') != -1) {
							...
							//image handling
								...
								reader.onloadend = function (e) {
									...
									document.body.appendChild(img)
									img.onload = function () {
										img_width = 640 //img.offsetWidth
										img_height = 480 // img.offsetHeight
				...
				var import_image_background = function (e) {
					e.stopPropagation()
					e.preventDefault()
					$('#workarea').removeAttr('style')
					$('#main_menu').hide()
				...
				// 新增导入背景图
				var img_import_background = $('<input id="tool_import_background_input" type="file" style="display: none">').change(import_image_background)
				$('#tool_import_background').show().prepend(img_import_background)
				$('#tool_import_background').click(function (e) {
					e.stopPropagation()
					$('#tool_import_background_input')[0].click()
				})
			}

			var updateCanvas = Editor.updateCanvas =
				function (center, new_ctr) {
			...
			function updateRulers (scanvas, zoom) {
				...
				for (var d = 0; d < 2; d++) {
					...
					// Create multiple canvases when necessary (due to browser
					// limits)
					if (ruler_len >= limit) {
...
//            real_d2 = Math.abs(real_d2);
						...
						// Change 1000s to Ks
						if (label !== 0 && label !== 1000 && label % 1000 ===
							0) {
							label = (label / 1000) + 'K'
						}
...
//      });
			...
			try {
				var json_encode = function (obj) {
					...
					if (window.JSON && JSON.stringify) return JSON.stringify(
						obj)
					var enc = arguments.callee //for purposes of recursion
					if (typeof obj == 'boolean' || typeof obj == 'number') {
						return obj + '' //should work...
					} else if (typeof obj == 'string') {
						//a large portion of this is stolen from Douglas
						...
						for (var i = 0; i < obj.length; i++) {
							obj[i] = enc(obj[i]) //encode every sub-thingy on
							// top
						}
						return '[' + obj.join(',') + ']'
					} else {
			...
			$(function () {
				...
				svgCanvas.selectOnly([canvas_background])
				moveToBottomSelected()
				// console.log('asdasdasdasdfassaddas')
				window.parent.postMessage('svgCanvasReady', '*');
			})
			...
			Editor.setLang = function (lang, allStrings) {
				...
				if (allStrings) {
					...
					$('#multiselected_panel div[id^=tool_align]').
						each(function () {
							$('#tool_pos' + this.id.substr(10))[0].title =
								this.title
						})
		...
		Editor.addExtension = function () {
			...
			// Note that we don't want this on Editor.ready since some
			// extensions may want to run before then (like server_opensave).
			$(function () {
