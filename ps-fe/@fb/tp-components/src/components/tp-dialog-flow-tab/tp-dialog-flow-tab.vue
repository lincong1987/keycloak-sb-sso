<template>
	<div class="tp-dialog-flow-content">
		<fb-dialog
			ref="tp-dialog-flow-tab"
			...
			<fb-steps :current="tabIndex" align="center" @on-step-click="changeTab">
				<fb-step v-for="item in tabSteps" :key="item.label" :label="item.label" :icon="item.icon"/>
			</fb-steps>

			<div class="steps-content">
				<component v-show="tabIndex == index" v-for="(item, index) in tabSteps" :key="index" :is="item.component" :param="dialog.param" :parentPage="currentPage"></component>
			</div>
	...
	export default {
		...
		methods: {
			...
			show(tabArry, param, title, options) {
				...
				this.$nextTick(() => {
					this.dialog.create && this.$refs['tp-dialog-flow-tab'].show()
				})
				// 每次打开清空参数
				this.pageParam = {}
			},
