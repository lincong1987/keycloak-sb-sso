	...
	methods: {
		...
		initView (param) {
			...
			if (param && param.ents && param.ents.length >= 1) {
				param.ents.forEach((node) => {
					checked.push(node.entId)
					list.push(node)
				})
		...
		add () {
			...
			this.closeTpDialog(this.list.map(n=>{
				n.label = n.entFullName
				n.value = n.entId
				return n
			}))
		},
		...
		handleRowSelect (selectId, selectedIds, selectRow, rowIndex, checked) {
			...
			if (selectRow) {
				...
				// 选中
				if (checked) {
					this.list.push(this.toRaw(selectRow))
				} else {
					// 取消选中remove
			...
			this.checked = this.list.map(n => n.entId)
