...
//      }
      ...
      for (var i=attrs.length-1; i>=0; i--) {
        ...
        if (attrVal != "") {
          ...
          if(!isNaN(attrVal) && attr.nodeName.indexOf('data-') < 0) {
            attrVal = svgedit.units.shortFloat(attrVal);
          } else if(unit_re.test(attrVal) && attr.nodeName.indexOf('data-') < 0) {
...
this.getBold = function() {
  ...
  this.attr = function(attr, value) {
    ...
    return typeof value === 'undefined'
        ? $(selectedElems).attr(attr)
        : $(selectedElems).attr(attr, value)
