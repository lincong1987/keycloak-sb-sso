	...
	methods: {
		...
		initView (param) {
			if (param && param.persons && param.persons.length >= 1) {
				param.persons.forEach((node) => {
					this.tags.push({
						label: node.label,
						value: node.value + ':' + node.deptId,
						deptId: node.deptId,
		...
		handleQuery () {
			...
			this.$svc.common.org.getPersonByDeptId(this.formData).then((result) => {
	...
	computed: {
		allChecked: {
			get () {
				...
				this.nodes.length ? true : 'indeterminate')
			},
			set (value) {
