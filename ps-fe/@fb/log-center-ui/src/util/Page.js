...
import permission from '@fb/fb-core/src/directive/permission/index.js'
	...
	methods: {
		...
		loadCityTreeByCodeData(item, callback) {
			if (item.children.length >= 1) {
				...
				this.$svc.sys.city.treeByCode({cityId: item.id, sync: 0, expand: true, showTop: 'on'}).then((result) => {
					if (result.code == 1) {
		...
		loadCityTreeData(item, callback) {
			if (item.children.length >= 1) {
				...
				this.$svc.sys.city.tree({cityId: item.id, sync: 0, expand: true, showTop: 'on'}).then((result) => {
					if (result.code == 1) {
						callback(result.data);
					} else {
						// 服务器返回失败
