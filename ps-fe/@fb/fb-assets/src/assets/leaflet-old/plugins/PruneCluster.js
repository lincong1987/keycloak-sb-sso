...
(function (PruneCluster_1) {
    ...
    var PruneCluster = (function () {
        ...
        PruneCluster.prototype.RegisterMarker = function (marker) {
            ...
            if (marker._latlng) {
                marker['position'] = marker._latlng
                marker['averagePosition'] = marker._latlng
                marker['hashCode'] = _hashCode++
                marker['weight'] = 1
        ...
        PruneCluster.prototype.eachLayer = function (callback) {
...
var PruneClusterForLeaflet = (L.Layer ? L.Layer : L.Class).extend({
        ...
        clusters.forEach(function (cluster) {
            ...
            if (oldMarker) {
                if (cluster.population === 1 && data._leafletOldPopulation === 1 && cluster.hashCode === oldMarker._hashCode) {
                    // console.log(cluster, cluster.lastMarker.data, 648)
                    if (resetIcons || oldMarker._zoomLevel !== zoom ) {
    ...
    eachLayer: function (callback) {
        return this.Cluster.eachLayer(callback);
    },
