...
// (function () {
// 	for (let a = 0, b = ['ms', 'moz', 'webkit', 'o'], c = 0; c < b.length &&
// 	!window.requestAnimationFrame; ++c) window.requestAnimationFrame =
// 		window[b[c] + 'RequestAnimationFrame'], window.cancelAnimationFrame =
// 		window[b[c] + 'CancelAnimationFrame'] ||
// 		window[b[c] + 'CancelRequestAnimationFrame']
// 	if (!window.requestAnimationFrame) window.requestAnimationFrame =
// 		function (b) {
// 			let c = (new Date).getTime(), f = Math.max(0, 16 - (c - a)),
...
CAV.Vector3 = {
	...
	divideScalar: function (a, b) {
		b !== 0 ? (a[0] /= b, a[1] /= b, a[2] /= b) : (a[0] = 0, a[1] =
			0, a[2] = 0)
		return this
	},
	cross: function (a, b) {
		let c = a[0], d = a[1], e = a[2]
		a[0] = d * b[2] - e * b[1]
		a[1] = e * b[0] - c * b[2]
		a[2] = c * b[1] - d * b[0]
		return this
	...
	limit: function (a, b, c) {
		let d = this.length(a)
		b !== null && d < b ? this.setLength(a, b) : c !== null && d > c &&
			this.setLength(a, c)
		return this
	},
	dot: function (a, b) {
	...
	distanceSquared: function (a, b) {
		let c = a[0] - b[0], d = a[1] - b[1], e = a[2] - b[2]
		return c * c + d * d + e * e
	},
...
CAV.Color.prototype = {
	...
	format: function () {
		let a = this.hexify(this.rgba[0]), b = this.hexify(this.rgba[1]),
...
CAV.Geometry.prototype = {
	update: function () {
		if (this.dirty) {
			let a, b
			for (a = this.triangles.length - 1; a >= 0; a--) b =
...
CAV.Plane = function (a, b, c, d) {
	...
	for (a = 0; a <= this.segments; a++) {
		c.push([])
		for (b = 0; b <= this.slices; b++) d =
			new CAV.Vertex(e + a * this.segmentWidth,
				f - b * this.sliceHeight), c[a].push(d), this.vertices.push(d)
	}
	...
	for (a = 0; a < this.segments; a++) {
		for (b = 0; b < this.slices; b++) {
			...
			let t0 = new CAV.Triangle(d, e, f)
			let t1 = new CAV.Triangle(f, e, g)
...
CAV.Mesh.prototype.update = function (a, b) {
	...
	if (b) for (c = this.geometry.triangles.length - 1; c >= 0; c--) {
		...
		for (e = a.length - 1; e >= 0; e--) f =
			a[e], CAV.Vector3.subtractVectors(f.ray, f.position,
			d.centroid), CAV.Vector3.normalise(f.ray), g =
			CAV.Vector3.dot(d.normal, f.ray), this.side === CAV.FRONT ? g =
			Math.max(g, 0) : this.side === CAV.BACK ? g =
			Math.abs(Math.min(g, 0)) : this.side === CAV.DOUBLE &&
			(g = Math.max(Math.abs(g), 0)), CAV.Vector4.multiplyVectors(
			this.material.slave.rgba,
			this.material.ambient.rgba, f.ambient.rgba), CAV.Vector4.add(
			d.color.rgba,
			this.material.slave.rgba), CAV.Vector4.multiplyVectors(
			this.material.slave.rgba, this.material.diffuse.rgba,
			f.diffuse.rgba), CAV.Vector4.multiplyScalar(
			this.material.slave.rgba, g), CAV.Vector4.add(d.color.rgba,
			this.material.slave.rgba)
...
CAV.Scene.prototype = {
	add: function (a) {
		a instanceof CAV.Mesh && !~this.meshes.indexOf(a)
			? this.meshes.push(a)
			: a instanceof CAV.Light && !~this.lights.indexOf(a) &&
			this.lights.push(a)
		return this
	},
	remove: function (a) {
		a instanceof CAV.Mesh && ~this.meshes.indexOf(a) ? this.meshes.splice(
			this.meshes.indexOf(a), 1) : a instanceof CAV.Light &&
...
CAV.Renderer.prototype = {
	setSize: function (a, b) {
		if (!(this.width === a && this.height === b)) return this.width =
			a, this.height = b, this.halfWidth =
...
CAV.CanvasRenderer.prototype.clear = function () {
	...
	this.context.clearRect(-this.halfWidth, -this.halfHeight, this.width,
		this.height)
	return this
}
CAV.CanvasRenderer.prototype.render = function (a) {
	...
	for (b = a.meshes.length - 1; b >= 0; b--) if (c = a.meshes[b], c.visible) {
		...
		for (d = c.geometry.triangles.length - 1; d >= 0; d--) e =
			c.geometry.triangles[d], f =
			e.color.format(), this.context.beginPath(), this.context.moveTo(
			e.a.position[0], e.a.position[1]), this.context.lineTo(
			e.b.position[0], e.b.position[1]), this.context.lineTo(
			e.c.position[0],
			e.c.position[1]), this.context.closePath(), this.context.strokeStyle =
			f, this.context.fillStyle =
			f, this.context.stroke(),
			this.context.fill()
	}
	return this
}
