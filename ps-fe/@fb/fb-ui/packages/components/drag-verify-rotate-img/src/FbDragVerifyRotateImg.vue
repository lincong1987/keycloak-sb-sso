<!-- 拖拽拼图验证 -->
<template>
	<div :class="[`${prefix}-drag-verify-content ${prefix}-drag-verify-rotate`]" :style="{width: width + 'px'}">
		<transition name="fade">
			<div v-show="imgShow" ref="imgBg" :class="`${prefix}-drag-verify-imgBg`" :style="dragVerifyImgBgStyle">
				<img :class="`${prefix}-drag-verify-imgBg__img`" :src="imgs.bg" alt="">
				<i @click="reset" :class="`${prefix}-icon ${prefix}-icon-refresh`"></i>
				<span ref="handlerImgSlot" :class="`${prefix}-drag-verify-imgBg__slot`">
					<img ref="imgSlot" width="100%" :src="imgs.slot" alt="">
				</span>
			</div>
		</transition>
		...
		methods: {
				...
				if (typeof this.action === "string" && this.action) {
					...
					axios.get(this.action, {
						...
						if (res.data && res.data.data) {
							...
							let { backgroundImage, sliderImage, y, clientUuid, bgImageWidth} = data
							this.imgs.bg = "data:image/png;base64," + backgroundImage
							this.imgs.slot = "data:image/png;base64," + sliderImage
							this.captchaConfig.currentScale = this.captchaConfig.moveEnd / bgImageWidth
							this.captchaConfig.clientUuid = clientUuid
						} else {
			...
			dragMoving (e) {
				...
				if (this.isMoving) {
					...
					if (_x > 0 && _x <= (this.width - this.height)) {
						...
						handlerImgSlot.style.transform = "rotate(" + (_x / (this.captchaConfig.moveEnd / 360)) + "deg)";
			...
			dragFinish (e) {
				if (this.isMoving) {
					...
					_x = Math.floor(_x / this.captchaConfig.currentScale)
					this.captchaConfig.entSlidingTime = new Date()
					this.$emit('on-verify', {x: _x, ...this.captchaConfig})
				}
			},
			// 进行中状态
			inStatus () {
			...
			reset () {
				this.$refs.handlerImgSlot.style.transform = "rotate(0deg)";
