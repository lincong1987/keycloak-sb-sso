<template>
	<div class="tp-dialog-step-content">
		<fb-dialog
			ref="tp-dialog-step-tab"
			...
			<fb-steps :current="tabIndex" align="center">
				<fb-step v-for="item in tabSteps" :key="item.label" :label="item.label" :icon="item.icon"/>
			</fb-steps>

			<div class="steps-content">
				<component ref="componentPage" v-show="tabIndex == index" v-for="(item, index) in tabSteps" :key="index"
						   :is="item.component" :param="dialog.param" :parentPage="currentPage"></component>
			</div>
...
export default {
	...
	data() {
		return {
			...
			dialog: {
				...
				top: '15vh',
				overflow: 'hidden',
				tabChangeConfirm: false, // step 不生效，无用
	...
	methods: {
		...
		show(tabArry, param, title, options) {
			...
			this.$nextTick(() => {
				this.dialog.create && this.$refs['tp-dialog-step-tab'].show()
			})
			// 每次打开清空参数
			this.pageParam = {}
		},
