	...
	<fb-container>
		<fb-page-search>
			...
			<template slot="buttons" v-if="search.props.show_buttons">
				<template v-if="search.actions.add.show">
					<fb-button ref="buttonAdd" :icon="search.actions.add.icon"
					           v-permission="mode==='test' ? {} :search.actions.add.code"
					           @on-click="handleAdd"
					>{{
				...
				<fb-button v-if="search.actions.export.show" ref="buttonExport" :icon="search.actions.export.icon"
				           ...
				           v-permission="mode==='test' ? {} :search.actions.export.code"
			...
			<template slot="actions" v-if="search.props.show_actions">
				<fb-button v-if="search.actions.query.show" type="primary" :icon="search.actions.query.icon"
				           ...
				           v-permission="mode==='test' ? {} :search.actions.query.code"
					...
					<template v-slot:op="props">
						<fb-space>
							<fb-button editor size="s"
							           ...
							           v-permission="mode==='test' ? {} :search.actions.modify.code"
							>
							...
							<fb-button size="s" v-show="search.actions.delete.show"
							           ...
							           v-permission="mode==='test' ? {} :search.actions.delete.code"
							>
...
import { arrayToObject, toRaw, uniqAndCleanArray } from '../../../../../views/sys/custom/form/form-editor/utils/utils'
...
// 列表方法
		...
		handleAdd () {
			...
			// 打开新增界面弹出窗
			this.$refs.TpDialog.show(import('../../../../../views/sys/custom/form/runtime-add.vue'), param, '新增', options)
		},
		...
		handleEdit (row) {
			...
			this.$refs.TpDialog.show(import('../../../../../views/sys/custom/form/runtime-add.vue'), param, '修改', options)
		},
		// 查看方法
		handleView (row) {
			let param = {
			...
			this.$refs.TpDialog.show(import('../../../../../views/sys/custom/form/runtime-view.vue'), param, '查看', options)
		},
		// 删除方法
		handleDel (row) {
