...
<script>
	...
	import CascaderMenu from './FbCascaderMenu';
	...
	const getSibling = (el, distance) => {
		const {parentNode} = el;
		if (parentNode) {
			const siblings = parentNode.querySelectorAll(`.${prefix}-cascader-node[tabindex="-1"]`);
	...
	export default {
		...
		methods: {
			initStore() {
				...
				if (config.lazy && isEmpty(data)) {
					this.lazyLoad();
				} else {
					this.store = new Store(data, config);
			...
			handleKeyDown(e) {
				...
				switch (keyCode) {
					...
					case KeyCode.left:
						...
						if (preMenu) {
							const expandedNode = preMenu.$el.querySelector(`.${prefix}-cascader-node[aria-expanded="true"]`);
					...
					case KeyCode.right:
						...
						if (nextMenu) {
							const firstNode = nextMenu.$el.querySelector(`.${prefix}-cascader-node[tabindex="-1"]`);
							focusNode(firstNode);
						}
						break;
					case KeyCode.enter:
			...
			handleExpand(node, silent) {
				...
				if (!silent) {
					...
					if (!valueEquals(pathValues, activePathValues)) {
						...
						this.$emit('on-expand-change', pathValues);
					}
				}
			},
