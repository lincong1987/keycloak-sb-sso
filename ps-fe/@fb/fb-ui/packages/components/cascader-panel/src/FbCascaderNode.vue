	...
	export default {
		...
		methods: {
			...
			renderPrefix(h) {
				...
				const {checkStrictly, multiple, changeOnSelect} = config;

				if (multiple) {
					return this.renderCheckbox(h);
				} else if (checkStrictly) {
					return this.renderRadio(h);
				} else if (changeOnSelect && isChecked) {
			...
			renderCheckbox(h) {
				...
				return h('fb-checkbox', {
					props: {
						value: node.indeterminate ? 'indeterminate' : node.checked,
						disabled: isDisabled
					},
					...events
				})
			...
			renderCheckIcon(h) {
				...
				// 		name: 'selected',
				// 	},
				// 	class: `${prefix}-icon-check ${prefix}-cascader-node__prefix`
				// })
			},
			...
			renderLoadingIcon(h) {
				return h('fb-icon', {
					...
					class: `${prefix}-icon-loading ${prefix}-cascader-node__postfix`
				})
			},
			...
			renderExpandIcon(h) {
				return h('fb-icon', {
					...
					class: `${prefix}-icon-arrow-right ${prefix}-cascader-node__postfix`
				})
			},
			...
			renderContent(h) {
				...
				return h('span', {
					class: [`${prefix}-cascader-node__label`]
				}, [vnode || node.label])
			}
		},
		...
		render(h) {
			...
			if (isLeaf && !isDisabled && !checkStrictly && !multiple ||
				changeOnSelect && !isDisabled && !checkStrictly && !multiple) {
