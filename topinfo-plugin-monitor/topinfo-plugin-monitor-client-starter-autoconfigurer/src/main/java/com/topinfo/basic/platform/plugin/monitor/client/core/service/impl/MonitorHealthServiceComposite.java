...
import com.topinfo.basic.platform.plugin.monitor.client.core.bean.ServerHealth;
import com.topinfo.basic.platform.plugin.monitor.client.core.service.MonitorHealthService;
import com.topinfo.basic.platform.plugin.monitor.client.core.util.MonitorClientThreadPool;
...
public class MonitorHealthServiceComposite implements ApplicationContextAware {
    ...
    private void removeExcludeSource(List<MonitorHealthService> list) {
        try {
            ...
            LOGGER.error("监控服务客户端启动后，删除排除的服务失败，请检查配置文件【topinfo.platform.plugin.monitor.exclude-source】");
        }
    }
}
