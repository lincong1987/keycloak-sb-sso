...
import FbFormEditorColAddRun from '../../../../../views/sys/custom/form/form-editor/FbFormEditorColAddRun'
...
//		let formProps = await this.$svc.sys.custom.module.view({mid}).then(res => res.data)
		let add
		if (mid) {
			add = await this.$svc.sys.custom.form.mid({
				mid,
				ftype: 'edit',
				fSource: 'pc',
			}).then(res => res.data)
		...
		if (mcode) {
			add = await this.$svc.sys.custom.form.mcode({
				mcode,
				ftype: 'edit',
				fSource: 'pc',
			}).then(res => res.data)
		...
		this.form.props = {
			mid: add.mid,
			name: 'name',
			code: add.fjson.replace('C_JSON_', ''),
		}
...
//		})
		...
		if (add === '' || add.fjson === '') {
			...
			this.add = JSON.parse(add.fjson)
		}

		if (add.fid) {
			this.add.props.fid = add.fid
		}
